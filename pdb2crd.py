...
_s = re.compile(r'\s+')   # fixed the regex warning
...
def write_crd(title, psf, pdb, crd):
    crd.write("\n".join(title))
    crd.write("\n*\n")
    crd.write("%10d  EXT\n" % len(psf))
    ...
    for i, atom in enumerate(psf):
        ...
        crd.write("  %8d  %8d  %-8s  %-8s  %18.10f  %18.10f  %18.10f  %-8s  %-8d  %18.10f\n" %
                  (atom['num'], resid, atom['resname'], atom['name'],
                   pdb[i][0], pdb[i][1], pdb[i][2], atom['segid'], atom['resid'], pdb[i][3]))
...
if __name__ == '__main__':
    ...
    if not args.output:
        ...
        crd = open(args.output, 'w')   # fixed typo here
